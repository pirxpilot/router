...
describe('OPTIONS', () => {
  it('should respond with defined routes', (_, done) => {
    ...
    request(server).options('/users').expect('Allow', 'GET, HEAD, POST, PUT').expect(200, 'GET, HEAD, POST, PUT', done);
  });
  ...
  it('should not contain methods multiple times', (_, done) => {
    ...
    request(server).options('/users').expect('GET, HEAD, PUT').expect('Allow', 'GET, HEAD, PUT', done);
  });
  ...
  describe('when error occurs in respone handler', () => {
    it('should pass error to callback', (_, done) => {
      const router = Router();
      const server = createServer(function hander(req, res, _next) {
        res.writeHead(200);
        router(req, res, err => {
